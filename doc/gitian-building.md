...
*Setup instructions for a Gitian build of TheHolyRoger Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the TheHolyRoger
...
- [Building TheHolyRoger Core](#building-theholyroger-core)
...
Clone the git repositories for TheHolyRogerCoin and Gitian.
...
in the TheHolyRogerCoin repository under 'Fetch and create inputs' to install sources which require
...
To build TheHolyRoger Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the TheHolyRogerCoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/TheHolyRogerCoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/TheHolyRoger/TheHolyRogerCoin
...
For example:
```bash
URL=https://github.com/thrasher-/TheHolyRogerCoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit TheHolyRogerCoin=${COMMIT} --url TheHolyRogerCoin=${URL} ../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit TheHolyRogerCoin=${COMMIT} --url TheHolyRogerCoin=${URL} ../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit TheHolyRogerCoin=${COMMIT} --url TheHolyRogerCoin=${URL} ../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the TheHolyRogerCoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../TheHolyRogerCoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/theholyroger
SIGPATH=/some/root/path/theholyroger-detached-sigs

./bin/gbuild --url TheHolyRogerCoin=${BTCPATH},signature=${SIGPATH} ../TheHolyRogerCoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
in `gitian.sigs.theholyroger` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/theholyroger-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/theholyroger-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/theholyroger-osx-build.assert
```
...
[gitian.sigs.theholyroger](https://github.com/theholyroger-project/gitian.sigs.theholyrogers/) repository, or if that's not possible create a pull
