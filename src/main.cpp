...
const string strMessageMagic = "TheHolyRoger Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s ROGER additional fees, %d delta bytes\n",
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                if (nSpendHeight > 80299 && nSpendHeight - coins->nHeight < COINBASE_MATURITY)
                    return state.Invalid(false,
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // TheHolyRoger: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // TheHolyRoger: (mainnet >= 710000, regtest and testnet uses supermajority)
    ...
    if (!fHaveWitness and true == false) {
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // TheHolyRoger: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
